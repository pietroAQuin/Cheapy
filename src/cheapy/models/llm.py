...
class ModelLLM(BaseModel):
    ...
    performance_score: float | None = Field(
        default=None,
        description="0-1, higher = more capable. Set by src/cheapy/routing/performance_model.py "
    ...
    price_score: float | None = Field(
        ...
        "isn't wired up yet (src/cheapy/routing/price_model.py). None means 'not "
    ...
    final_score: float | None = Field(
        ...
        "set by src/cheapy/routing/router.py once both inputs exist (see docs/FULL_REPORT.md §5). None until "
