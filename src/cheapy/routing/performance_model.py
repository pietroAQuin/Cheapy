"""`performance_model.py` — sets `ModelLLM.performance_score`, per docs/FULL_REPORT.md §2/§5.

Thin adapter only. All the scoring logic — pairwise divergence, prior-weighted
conformity, the ridge regressor — lives in `research/capability_fitting/`, whose design is
recorded in `docs/capability_model.md`. This module exists
solely to satisfy the stage contract every `src/cheapy/routing/` file follows: a function of
`(Trajectory, list[ModelLLM]) -> list[ModelLLM]` that enriches the given `ModelLLM`
objects in place (docs/FULL_REPORT.md §2, "The scoring chain").

Never reads `trajectory.served_model` (docs/FULL_REPORT.md §5's "Shared contract") — neither this
module nor `capability_model.score_for_trajectory` touches it for scoring; it is read
only inside `research/capability_fitting/` as an explicit, logged diagnostic (spec §1.0,
§8), never as a feature.
"""

from __future__ import annotations

from cheapy.capability.capability_model import DEFAULT_BETA, score_for_trajectory
...
#: Measured over 200 trajectories (`research/capability_fitting/` sweep):
#:
