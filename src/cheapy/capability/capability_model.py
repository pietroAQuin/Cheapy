...
No API calls, no network, milliseconds. `src/cheapy/routing/performance_model.py` is the
intended caller.
...
constants so `src/cheapy/routing/router.py` integration is unblocked (§9.3). Once `train.py`
...
from cheapy.capability.features import extract_step_features, pair_descriptor
from cheapy.capability.priors import BASE_CAPABILITY, CANDIDATES, NEAR_UNMEASURED, PROBES, weights
...
#: re-exported by `src/cheapy/routing/performance_model.py` as `DEFAULT_BETA` — see that module
...
def score_for_trajectory(
    ...
    """Adapter for `src/cheapy/routing/performance_model.py`: sets `performance_score` in place
    on each `ModelLLM` and returns the list (docs/FULL_REPORT.md §2's stage contract). Models outside the
