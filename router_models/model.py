"""`model.py` — the last stage: weighted aggregation, ranking, HOLD / CHANGE TO.

Per README §2 this stage takes the `ModelLLM` objects the two scoring stages already
enriched and combines their scores into one number::

    final_score = (w_price · price_score + w_perf · performance_score) / (w_price + w_perf)
...
**Weights are arguments, never module constants** (README §5, "Aggregation"): sweeping
...
from router_models.performance_model import DEFAULT_BETA, score_performance
from router_models.price_model import score_price

__all__ = ["RoutingDecision", "aggregate_scores", "rank", "decide", "route"]
...
class RoutingDecision:
    ...
    action: Action
    label: str
    """`"HOLD"` or `"CHANGE TO <model>"` — the README §1 verdict string."""
...
def aggregate_scores(
    models: list[ModelLLM], *, w_price: float, w_performance: float
) -> list[ModelLLM]:
    ...
    """
    if w_price < 0 or w_performance < 0:
        raise ValueError(f"weights must be >= 0, got {w_price} / {w_performance}")
    total = w_price + w_performance
    if total == 0:
        raise ValueError("w_price + w_performance must be > 0 — no ranking is defined at 0/0")
    ...
    for model in models:
        if model.price_score is None or model.performance_score is None:
            ...
            continue
        model.final_score = (
            w_price * model.price_score + w_performance * model.performance_score
        ) / total
    return models
...
def decide(
    ...
    return RoutingDecision(
        ...
        ranking=tuple(m.name for m in ranked),
...
def route(
    ...
    *,
    ...
    """The whole scoring chain for one trajectory, in README §2's order.
    ...
    aggregate_scores(models, w_price=w_price, w_performance=w_performance)
