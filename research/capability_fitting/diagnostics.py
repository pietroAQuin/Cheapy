...
than a fixable one.
    ...
    python -m research.capability_fitting.diagnostics \
...
from research.capability_fitting.train import (
    ALL_FEATURE_NAMES,
    actions_for_step,
    build_dataset,
    load_samples,
...
from research.capability_fitting.elicit import load_store
...
def same_model_vs_cross_model(samples_path: Path, responses_path: Path) -> str:
    ...
    samples = load_samples(samples_path)
    store = load_store(responses_path)
    from research.capability_fitting.scoring import pair as pair_score
