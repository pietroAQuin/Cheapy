...
# Family-level capability prior, 0-1. Deliberately flat within a family (e.g.
# claude-opus-5 vs claude-opus-4-8 vs claude-opus-4-6 all score the same) —
# ordering generations within a family would be another unstated assumption
# stacked on an already-assumed tier order. Leave that to the empirical pass.
_FAMILY_CAPABILITY_PRIOR: dict[str, float] = {
    "claude-opus": 1.0,
    "claude-sonnet": 0.6,
    "gpt-5.6": 0.6,  # no published tier vs. sonnet — parked at the same prior
    "claude-fable": 0.2,
}
...
def build_model_list() -> list[ModelLLM]:
    """Return one ModelLLM per known model id, capability seeded, price unset."""
    return [
        ModelLLM(
            name=name,
            family=family,
            capability_score=_FAMILY_CAPABILITY_PRIOR[family],
        )
